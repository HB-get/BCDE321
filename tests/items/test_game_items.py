...
class TestGameItems:
    def test_an_empty_pocket_reads_as_nothing(self) -> None:
        assert GameItems().held_items() == (ItemCode.NONE, ItemCode.NONE)

    def test_held_items_reports_codes_not_names(self) -> None:
        assert carrying("machete").held_items() == (ItemCode.MACHETE, ItemCode.NONE)

    def test_both_pockets_are_reported(self) -> None:
    ...
    def test_oil_is_reported_when_carried(self) -> None:
        assert carrying("oil").get_has_oil() is True

    def test_oil_is_not_reported_when_absent(self) -> None:
        assert carrying("machete").get_has_oil() is False

    def test_candle_and_gasoline_is_an_instant_kill(self) -> None:
        assert carrying("candle", "gasoline").get_has_instant_kill() is True

    def test_candle_and_oil_is_also_an_instant_kill(self) -> None:
        assert carrying("candle", "oil").get_has_instant_kill() is True

    def test_a_candle_alone_is_not_an_instant_kill(self) -> None:
        assert carrying("candle").get_has_instant_kill() is False

    def test_fuel_without_a_candle_is_not_an_instant_kill(self) -> None:
        assert carrying("gasoline", "oil").get_has_instant_kill() is False

    # attack_bonus

    def test_attack_bonus_comes_back_as_a_successful_result(self) -> None:
        result = carrying("machete").attack_bonus(with_chainsaw=False)

        assert result.is_fail() is False
        assert result.get_data() == 2

    def test_declining_the_chainsaw_uses_the_next_best_weapon(self) -> None:
        result = carrying("chainsaw", "machete").attack_bonus(with_chainsaw=False)

        assert result.get_data() == 2

    def test_swinging_a_chainsaw_you_do_not_have_is_refused(self) -> None:
    ...
    def test_swinging_a_spent_chainsaw_is_refused(self) -> None:
    ...
    def test_a_found_item_is_not_carried_until_it_is_kept(self) -> None:
        items = GameItems()
    ...
    def test_keeping_a_found_item_puts_it_in_a_pocket(self) -> None:
        items = GameItems()
        items.find_item(ItemCode.MACHETE)
    ...
    def test_keeping_nothing_is_refused(self) -> None:
        assert GameItems().keep_found_item() is ErrorCode.NO_FOUND_ITEM

    def test_keeping_an_empty_find_is_refused(self) -> None:
    ...
    def test_keeping_an_item_with_no_pocket_free_is_refused(self) -> None:
    ...
    def test_a_refused_item_can_still_be_kept_once_a_pocket_frees_up(self) -> None:
    ...
    def test_walking_away_from_a_found_item_forgets_it(self) -> None:
        items = GameItems()
        items.find_item(ItemCode.MACHETE)
    ...
    def test_discarding_a_slot_empties_it(self) -> None:
        items = carrying("machete", "chainsaw")

        assert items.discard(1) is None
        assert items.held_items() == (ItemCode.MACHETE, ItemCode.NONE)

    def test_discarding_an_empty_slot_is_refused(self) -> None:
    ...
    def test_using_a_soda_spends_it(self) -> None:
        items = carrying("can_of_soda")

        assert items.use(ItemCode.SODA) is None
        assert items.held_items() == (ItemCode.NONE, ItemCode.NONE)

    def test_using_a_soda_you_do_not_have_is_refused(self) -> None:
        assert carrying("machete").use(ItemCode.SODA) is ErrorCode.NO_SODA

    def test_using_gasoline_refuels_the_chainsaw(self) -> None:
    ...
    def test_using_gasoline_spends_it(self) -> None:
        items = carrying("chainsaw", "gasoline")
    ...
    def test_using_gasoline_you_do_not_have_is_refused(self) -> None:
        assert carrying("chainsaw").use(ItemCode.GASOLINE) is ErrorCode.NO_GASOLINE

    def test_using_gasoline_without_a_chainsaw_is_refused_and_keeps_it(self) -> None:
    ...
    def test_using_oil_is_refused_and_keeps_it(self) -> None:
    ...
    def test_using_a_weapon_is_refused_and_keeps_it(self) -> None:
        items = carrying("machete")
    ...
    def test_reset_empties_the_pockets(self) -> None:
        items = carrying("machete", "chainsaw")
    ...
    def test_reset_refills_the_chainsaw(self) -> None:
        items = carrying("chainsaw")
        for _ in range(Inventory.CHAINSAW_BATTLES):
            items.record_battle()
    ...
    def test_reset_forgets_a_found_item(self) -> None:
        items = GameItems()
        items.find_item(ItemCode.MACHETE)
    ...
    def test_battles_burn_chainsaw_fuel_through_the_adapter(self) -> None:
        items = carrying("chainsaw")
