...
class TestItemIds:
    def test_every_implemented_item_is_a_rulebook_item(self) -> None:
        assert IMPLEMENTED_ITEMS <= set(RULEBOOK_ITEMS)

    def test_implemented_items_match_what_the_inventory_actually_knows(self) -> None:
        known = set(WEAPON_ATTACK) | set(CONSUMABLE_EFFECTS)

        assert IMPLEMENTED_ITEMS == known

    def test_rulebook_item_names_are_unique(self) -> None:
        assert len(set(RULEBOOK_ITEMS)) == len(RULEBOOK_ITEMS)

    def test_every_mapped_card_produces_a_rulebook_item(self) -> None:
        """Guards the mapping as it is filled in, one card at a time."""
        assert set(CARD_ITEM_IDS.values()) <= set(RULEBOOK_ITEMS)

    def test_no_two_cards_map_to_the_same_item(self) -> None:
        assert len(set(CARD_ITEM_IDS.values())) == len(CARD_ITEM_IDS)

    def test_an_unmapped_card_is_refused_rather_than_guessed(self) -> None:
